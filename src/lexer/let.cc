...
void fell::lex::let(const std::string_view statement) {
    std::size_t i = 4;
    ...
    if(i == statement.length()) {
        (*lang::global_table)[std::string{name}] = util::make_var<types::nihil>();
    } else {
        ...
        (*lang::global_table)[std::string{name}] = solve_expression(expr);
    }
}

void fell::lex::check_for_invalid_redefinition(const std::string_view name) {
    ...
    if(name[0] == '{')
        throw std::runtime_error{"Attempt to initialize rvalue."};
