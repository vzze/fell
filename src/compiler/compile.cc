...
std::vector<std::int32_t> * fell::compiler::instructions::top([[maybe_unused]] const bool push, [[maybe_unused]] const std::size_t i) {
    if(ins.empty()) {
        if(push) ref->main_program.first.push_back(data->locations[i]);

        return &ref->main_program.second;
    } else {
            ...
            case RIGHT_CURLY: {
                ...
                switch(ctx_solver.contexts.top()) {
                    ...
                    case CONTEXT::WHILE: {
                        ...
                        instructions.top(true, i)->push_back(-static_cast<std::int32_t>(index - instructions.top()->size() + static_cast<std::size_t>((*instructions.top())[index - 1])) - 1);
                    ...
                    case CONTEXT::DO_WHILE: {
                        ...
                        instructions.top(true, i)->push_back(-static_cast<std::int32_t>(index - instructions.top()->size()));
